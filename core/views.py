        ...
        if UploadData.objects.filter(imei=imei).exists():
            ...
            if request.method == "POST":
                ...
                if stored_data.password == entered_password:
                    ...
                    return render(request, "base/password_popup.html", { "imei": imei,"show_contact_popup": True})
            return render(request, "base/password_popup.html", {"imei": imei,"show_contact_popup": False})
...
def check_password_status(request):
    imei = request.GET.get("imei")
    if not imei:
        return JsonResponse({"error": "IMEI parameter is missing"}, status=400)
    try:
        user_setting = UploadData.objects.get(imei=imei)  # Fetch from DB
        return JsonResponse({"password_saved": True, "password": user_setting.password})
    except UploadData.DoesNotExist:
        return JsonResponse({"password_saved": False})
...
def SaveData(request):
    if request.method == "POST":
        ...
        if imei:
            ...
            if existing_data:
                # If any of the existing media fields have values, clear them
                if existing_data.video or existing_data.photo or existing_data.recording:
                    existing_data.video = None
                    existing_data.photo = None
                    existing_data.recording = None

                # Update with new values (if provided)
                existing_data.video = video if video else None
                existing_data.photo = image if image else None
                existing_data.recording = audio if audio else None
                existing_data.is_name = is_name if is_name else existing_data.is_name
                existing_data.is_comments = is_comments if is_comments else existing_data.is_comments
                existing_data.password_protected = is_password
                existing_data.password = request.POST.get("password") if is_password else existing_data.password
                
                existing_data.save()

                logger.info("Updated existing record for IMEI: %s", imei)
                messages.success(request, "Data updated successfully")
                return redirect("core:output-data", slug=existing_data.slug)

            else:
                # Create a new record if IMEI is not found
                ...
                logger.info("Created new record for IMEI: %s", imei)
                messages.success(request, "Data saved successfully")
                return redirect("core:output-data", slug=upload.slug)

        # Store name and comments in session
